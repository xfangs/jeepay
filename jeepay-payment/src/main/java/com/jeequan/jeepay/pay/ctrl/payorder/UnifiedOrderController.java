...
import com.jeequan.jeepay.core.utils.IdGeneratorSnowflake;
...
public class UnifiedOrderController extends AbstractPayOrderController {
   ...
   * 统一下单接口
   **/
  @PostMapping("/api/pay/createOrder")
  public ApiRes createOrder(@RequestBody UnifiedOrderRQ rq) {

    log.info("进入统一下单");

    MchApp mchApp = mchAppService.getOne(
        new QueryWrapper<MchApp>().eq("mch_no", rq.getMchNo()));

    rq.setClientIp(getClientIp());
    rq.setAppId(mchApp.getAppId());
    rq.setReqTime(System.currentTimeMillis() + "");
    rq.setMchOrderNo("M" + idGeneratorSnowflake.snowflakeId());
