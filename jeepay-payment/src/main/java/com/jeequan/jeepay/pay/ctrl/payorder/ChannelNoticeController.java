...
public class ChannelNoticeController extends AbstractCtrl {
  ...
  public String doReturn(HttpServletRequest request, @PathVariable("ifCode") String ifCode,
    ...
    String logPrefix =
        "进入[" + ifCode + "]支付同步跳转：urlOrderId：[" + StringUtils.defaultIfEmpty(urlOrderId, "")
            + "] ";
    log.info("===== {} =====", logPrefix);
    ...
    try {
      ...
      // 解析订单号 和 请求参数
      MutablePair<String, Object> mutablePair = payNotifyService.parseParams(request, urlOrderId,
      ...
      if (notifyResult == null || notifyResult.getChannelState() == null
        ...
        log.error("{}, 处理回调事件异常  notifyResult data error, notifyResult ={} ", logPrefix,
            notifyResult);
  ...
  public ResponseEntity doNotify(HttpServletRequest request, @PathVariable("ifCode") String ifCode,
    ...
    try {
      ...
      log.info("payOrder.getState():{}", payOrder.getState());
