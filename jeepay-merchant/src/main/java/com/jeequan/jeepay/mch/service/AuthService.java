...
public class AuthService {
  ...
  public String auth(String username, String password) {
    ...
    try {
      ...
      throw jex.getBizException() == null ? new BizException(jex.getMessage())
          : jex.getBizException();
    ...
    if (sysUser.getIsAdmin() != CS.YES
        && sysEntitlementMapper.userHasLeftMenu(sysUser.getSysUserId(), CS.SYS_TYPE.MCH) <= 0) {
    ...
    UsernamePasswordAuthenticationToken authenticationRest = new UsernamePasswordAuthenticationToken(
        jeeUserDetails, null, jeeUserDetails.getAuthorities());
  ...
  public void refAuthentication(List<Long> sysUserIdList) {
    ...
    for (Long sysUserId : sysUserIdList) {
      ...
      for (String cacheKey : cacheKeyList) {
        ...
        //用户不存在 || 已禁用 需要删除Redis
        if (sysUserMap.get(sysUserId) == null
            || sysUserMap.get(sysUserId).getState() == CS.PUB_DISABLE) {
  ...
  public List<SimpleGrantedAuthority> getUserAuthority(SysUser sysUser) {
    ...
    List<String> entList = sysRoleEntRelaService.selectEntIdsByUserId(sysUser.getSysUserId(),
        sysUser.getIsAdmin(), sysUser.getSysType());
