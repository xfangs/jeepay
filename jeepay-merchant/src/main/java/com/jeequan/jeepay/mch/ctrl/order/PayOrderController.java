...
public class PayOrderController extends CommonCtrl {
  ...
  public ApiRes list() {
    ...
    List<PayInterfaceDefine> payInterfaceDefineList = payInterfaceDefineService.listByIds(
        pages.getRecords().stream().map(PayOrder::getIfCode).collect(
            Collectors.toList()));
    ...
    for (PayOrder order : pages.getRecords()) {
      ...
      PayInterfaceDefine payInterfaceDefine = payInterfaceDefineList.stream()
          .filter(i -> i.getIfCode().equals(order.getIfCode())).findFirst().get();

      order.addExt("appIcon", payInterfaceDefine.getAppIcon());

      order.addExt("status_dictText",
          dictItemService.getDictText("order_status", order.getState() + ""));
  ...
  public ApiRes sum() {
    ...
    Map<String, Object> result = payOrderService.getMap(new QueryWrapper<PayOrder>()
        .select("sum(amount) amount")
        .eq("mch_no", getCurrentMchNo())
        .eq("state", 2)
        .like("created_at", getValStringRequired("date"))
    );
  ...
  public ApiRes refund(@PathVariable("payOrderId") String payOrderId) {
    ...
    JeepayClient jeepayClient = new JeepayClient(
        sysConfigService.getDBApplicationConfig().getPaySiteUrl(), mchApp.getAppSecret());
